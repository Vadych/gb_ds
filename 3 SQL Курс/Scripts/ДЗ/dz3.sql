...
-- 1-1  
-- Не заню, можно ли такую связь назвать 1 - 1, но подразумевалось
-- что на каждое сообщество может быть только 1 чат

DROP TABLE IF EXISTS chats;
CREATE TABLE chats(
	id SERIAL PRIMARY KEY,
	name varchar(255),
	communities_id BIGINT UNSIGNED UNIQUE,
...
-- 1-1 по отношению к чату и 1 - М по отношению к медиа и модератору  
...
CREATE TABLE chat_settings(
	...
	FOREIGN KEY (chat_id) REFERENCES chats(id) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (moderator_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (icon_media_id) REFERENCES media(id) ON UPDATE CASCADE ON DELETE SET NULL
);
